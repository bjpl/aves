...
export const EnhancedLearnPage: React.FC = () => {
  ...
  const [selectedAnnotation, setSelectedAnnotation] = useState<{ id: string; term: string; english: string; pronunciation: string; x: number; y: number; description: string } | null>(null);
  ...
  const birdLearningData = useMemo(() => {
    ...
    const contentByImage = apiContent.reduce((acc, item) => {
      ...
      const bbox = item.boundingBox as { topLeft?: { x: number; y: number }; x?: number; y?: number; width: number; height: number };
      if (bbox.topLeft && typeof bbox.topLeft.x === 'number') {
        // New nested format
        startX = bbox.topLeft.x;
        startY = bbox.topLeft.y;
      } else if (typeof bbox.x === 'number' && typeof bbox.y === 'number') {
        // Old flat format
        startX = bbox.x;
        startY = bbox.y;
      } else {
  ...
  const handleAnnotationClick = async (annotation: { id: string; term: string; english: string; pronunciation: string; x: number; y: number; description: string }) => {

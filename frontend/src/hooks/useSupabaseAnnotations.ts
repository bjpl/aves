 ...
 */
export const useAnnotationSubscription = (imageId: string, callback: (payload: unknown) => void) => {
  useQuery({
    ...
    queryFn: () => {
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            info('Real-time annotation update', { imageId, event: (payload as { eventType?: string }).eventType });
            callback(payload);
          }
        )
        .subscribe();

...
import { Species, SpeciesFilter } from '../types';
import { api } from '../services/apiAdapter';
import { queryKeys } from '../config/queryClient';
import { error as logError } from '../utils/logger';

// Hook: Fetch all species with optional filters
export const useSpecies = (filters?: SpeciesFilter) => {
  return useQuery({
    queryKey: queryKeys.species.list(filters as Record<string, unknown>),
    queryFn: async () => {
      try {
...
export const usePrefetchSpecies = () => {
  ...
  return {
    prefetchSpecies: (filters?: SpeciesFilter) => {

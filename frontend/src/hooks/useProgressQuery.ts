...
export const useRecordTermDiscovery = () => {
  ...
  return useMutation({
    ...
    onError: (err, term, context) => {
      ...
      logError('Error recording term discovery', err as Error);
    },
    onSuccess: () => {
...
export const useRecordExerciseCompletion = () => {
  ...
  return useMutation({
      ...
      if (previousProgress) {
        queryClient.setQueryData<UserProgress>(
          queryKeys.progress.session(sessionId),
          {
            ...
            correctAnswers: correct ? previousProgress.correctAnswers + 1 : previousProgress.correctAnswers,
            totalAnswers: previousProgress.totalAnswers + 1,
    ...
    onError: (err, variables, context) => {
      ...
      logError('Error recording exercise completion', err as Error);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.progress.stats(sessionId) });
    },

/**
 * Vision AI Service - GPT-4o Integration for Bird Anatomy Annotation
 *
 * Leverages OpenAI's GPT-4 Vision model to automatically generate
...
/**
 * Raw annotation response from GPT-4o
 */
interface GPTAnnotationResponse {
  spanishTerm: string;
  englishTerm: string;
  boundingBox: {
    x: number;
 ...
 *
 * Handles GPT-4o Vision API integration for automatic bird anatomy annotation
 */
export class VisionAI {
  ...
  // Structured prompt for GPT-4o bird anatomy annotation
...
Return ONLY the JSON array with 3-8 annotations depending on what's clearly visible. Do not include explanations or additional text.`;
  ...
  constructor(pool: Pool, config?: VisionAIConfig) {
    ...
    logger.info('VisionAI service initialized', {
  ...
  /**
   * Generate annotations for a bird image using GPT-4o Vision
   *
  ...
  async annotateImage(imageUrl: string, imageId: string): Promise<Annotation[]> {
    logger.info('Starting image annotation', { imageUrl, imageId });
    ...
    try {
      ...
      const gptResponse = await this.callGPTWithRetry(imageUrl);

      // Parse and validate response
      const rawAnnotations = this.parseAnnotations(gptResponse);
      ...
      logger.info('Successfully generated annotations', {
        imageUrl,
      ...
      logger.error('Failed to annotate image', error instanceof Error ? error : { error });
      throw error;
    }
  }
   ...
   * @returns Raw JSON string response from GPT
   */
  private async callGPTWithRetry(imageUrl: string): Promise<string> {
    let lastError: Error | null = null;

    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        logger.debug(`GPT-4o API call attempt ${attempt}/${this.config.maxRetries}`, { imageUrl });
        ...
        logger.debug('GPT-4o API call successful', {
          imageUrl,
        ...
        logger.warn(`GPT-4o API call failed (attempt ${attempt}/${this.config.maxRetries})`, {
    ...
    throw lastError || new Error('Failed to call GPT-4o after all retries');
  }

  /**
   * Parse JSON response from GPT-4o
   *
   * @param response - Raw JSON string from GPT
   * @returns Array of parsed annotation objects
   */
  private parseAnnotations(response: string): GPTAnnotationResponse[] {
    try {
      // GPT-4o might return wrapped in an object or as an array
   ...
   */
  private validateAnnotation(annotation: GPTAnnotationResponse): boolean {
    try {
      // Required fields
  ...
  /**
   * Convert GPT responses to full Annotation objects
   *
   * @param responses - Array of validated GPT responses
   * @param imageId - Database ID for the image
   * @returns Array of complete Annotation objects
   */
  private convertToAnnotations(responses: GPTAnnotationResponse[], imageId: string): Annotation[] {
    const now = new Date();
